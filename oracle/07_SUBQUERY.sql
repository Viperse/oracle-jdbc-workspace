...
-- 전지연 사원이 속해 있는 부서원들의 사번, 사원명, 전화번호, 직급명, 부서명, 입사일 조회 (단, 전지연 사원은 제외)
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, CASE WHEN EMP_ID IN (SELECT MANAGER_ID FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL) THEN '사수' ELSE '사원' END 구분
FROM EMPLOYEE;

/*
    비교대상 > ANY(서브쿼리) : 여러 개의 결과값 중에서 "한 개라도" 클 경우 (여러 개의 결과값 중에서 가장 작은 값보다 클 경우)
    비교대상 < ANY(서브쿼리) : 여러 개의 결과값 중에서 "한 개라도" 작을 경우 (여러 개의 결과값 중에서 가장 큰 값보다 작을 경우)
*/
...
/*
    비교대상 < ALL(서브쿼리) : 여러 개의 "모든" 결과값들보다 클 경우
    비교대상 > ALL(서브쿼리) : 여러 개의 "모든" 결과값들보다 작을 경우
*/
...
/*
    다중행 다중열 서브쿼리
     - 서브쿼리의 조회 결과값이 여러 행, 여러 열일 경우 (여러 행, 여러 열)
*/
...
-- 전 직원 중 급여가 가장 높은 상위 5명 순위, 급여 조회
...
FROM (SELECT DEPT_CODE, ROUND(AVG(NVL(SALARY, 0))) "AVG_SALARY"
      FROM EMPLOYEE
      GROUP BY DEPT_CODE
