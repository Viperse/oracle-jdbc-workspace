    ...
    3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 등 
     ...
     - SQL종류
       1. DDL(Data Definition Language) : 데이터 정의어
          - DBMS 의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어
           (CREATE 생성, ALTER 수정, DROP 삭제) - 테이블을 생성하고 수정하는 것
       2. DML(Data Manipilation Language) : 데이터 조작어(2.그 다음 많이 사용)
          - 실제 데이터를 조작하기 위해 사용하는 언어
          (INSERT 삽입, UPDATE 수정, DELETE 삭제) - 데이터를 삽입 , 수정, 삭제
       3. DQL(Data Query Language)  : 데이터 질의어 (1.가장 많이 사용)
          - 데이터를 조회(검색)하기 위해 사용하는 언어(SELECT)
       4. DCL(Data Control Language) : 데이터 제어어
          - DBMS에 대한 보안, 무결성, 복구 등 DBMS 를 제어하기 위한 언어
           (GRANT 권한 부여, REVOKE 권한 회수)
       5. TCL(Ttansaction Control Language) : 트랜잭션을 제어하는 언어
 ...
 */
 ...
 /*
   ...
   - SELECT 를 통해서 조회된 결과를 RESULT SET 이라고 한다. (즉, 조회된 행들의 집합)
 ...
 */
 ...
 -- EMPLOYEE 테이블의 전체 사원들의 사번(EMP_ID), 이름(EMP_NAME), 급여만(SALARY)을 조회
 ...
 */
 ...
 --EMPOLIYEE에 사원명, 입사일, 근무일수(오늘날짜 - 입사일) 조회
 --오늘날짜 :  SYSDATE 
 ...
 */
 ...
 -- XXX(사원명)의 월급은 XXX(급여)원 입니다. 별칭 : "급여 정보" 
 ...
 */
 ...
 -- WHRER 절에서 OR(또는), AND(그리고)도 사용할 수 있음
 ...
 */
 ...
 -- ESCAPE OPTION : 나만의 와일드카드를 만들어서 사용!
...
-- 또는  WHERE NOT EMAIL LIKE '___$_%' ESCAPE '$'; (NOT의 위치 2가지 )
...
/*
 ...
 비교대상컬럼 IN ('값1', '값2',...)
*/
-- 부서코드가 D6이거나 D8이거나 D5가 아닌 부서원들의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;
 ...
 -- 2. 연봉(보너스포함X)이 3000만원 이상이고 보너스를 받지 않은 사원들의 사번, 사원명, 급여,
 --    보너스 조회
 SELECT EMP_ID, EMP_NAME, SALARY, BONUS
 FROM EMPLOYEE
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명,
--    급여, 보너스 포함연봉 조회(별칭)
 ...
 /*
  ...
  - NULLS FIRST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우
                  해당 데이터를 맨 앞 배치(생략 시 DESC일때 기본 값)
  - NULLS LAST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤에 배치
                 (생략 시 ASC일 때 기본 값)
 */
 ...
 FROM EMPLOYEE
 WHERE BONUS IS NOT NULL -- 보너스 NULL인 사람들 없앰
-- ORDER BY BONUS;
--ORDER BY BONUS DESC; -- 보너스 기준 내림차순 정렬
ORDER BY BONUS DESC NULLS LAST; --NULL이 있을 경우 해당 데이터를 맨 뒤에 배치