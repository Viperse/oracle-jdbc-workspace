/*
    ...
    EX)
    - 회원탈퇴 시 기존의 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
...
*/
-- 1. 문장 트리거
-- EMPLOYEE 테이블에 새로운 행이 INSERT될 때 '신입사원이 입사했습니다!' 메시지를 자동으로 출력하는 트리거 생성
...
-- EMPLOYEE 테이블에 UPDATE 수정 후 '업데이트 실행' 메시지를 자동으로 출력
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '갤럭시Z 플립 4', '삼성', 1000000, 10);
...
-- TB_PRODETAIL 테이블에 INSERT 이벤트 발생 시 TB_PRODUCT 테이블에 매번 자동으로 재고수량 UPDATE 되게끔 트리거 정의
-- 트리거명 : TRG_03
