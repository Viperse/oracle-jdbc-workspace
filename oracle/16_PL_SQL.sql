/*
     ...
     * PL/SQL 구조
      ...
      - [예외처리부(EXCEPTION SECTION)] : EXCEPTION으로 시작, 예외발생 시 해결하기 위한 구문을 미리 기술해 둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
/*
    1-2) 레퍼런스 타입 변수 선언 및 초기화 (어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서 그 타입으로 지정)
...
-- 문제로 풀어 보기!
-- 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL를 선언하고 각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY 컬럼과 DEPARTMENT 테이블의 DEPT_TITLE 컬럼의
-- 자료형을 참조! 사용자가 입력한 사번과 일치하는 사원을 조회한 후 조회 결과를 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
*/
-- 사번을 입력받은 후 해당 사원의 사번(EID), 이름(ENAME), 급여(SAL), 보너스(BONUS)를 출력 (단, 보너스를 받지 않는 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다.'라는 문구를 출력)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
...
-- 일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고 NCODE 값이 KO일 경우 => TEAM에 '국내팀' 대입, 그게 아닐 경우 TEAM에 '해외팀' 대입
...
-- 사용자에게 입력받은 사원과 일치하는 사번의 급여 조회 후 출력 (조회한 급여는 SAL 변수에 대입)
...
/*
    1-4) CASE 구문
        [표현식]
        결과값 대입할 변수 := CASE 비교대상자
            WHEN 비교값1 THEN 결과값1
            WHEN 비교값2 THEN 결과값2
            ESLE 결과값n
        END;
*/
-- 사번을 입력받은 후 사원의 모든 컬럼 데이터를 EMP에 대입하고 DEPT_CODE에 따라서 알맞는 부서를 출력 (DNAME 데이터타입은 VARCHAR2(30))
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8인 경우 기술지원부, D9인 경우 총무부
-- 나머지는 부서 없음
...
EXCEPTION
    ...
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('에러가 발생했습니다, 관리자에 문의해 주세요.');
END;
/
