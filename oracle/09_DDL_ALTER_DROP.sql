...
/*
    1-2) 컬럼 수정 (MODIFY)
        - 데이터 타입을 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 변경할 데이터타입;
        - 기본값 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 변경할 기본값;
          --> DEFAULT NULL : DEFAULT 삭제
*/
-- DEPT_ID 컬럼의 데이터 타입으로 CHAR(3)으로 변경
...
ALTER TABLE DEPT_COPY MODIFY CNAME NUMBER; --> 값이 없으면 데이터 타입 변경 가능!

-- 다중 수정
-- DEPT_COPY 테이블에서 DEPT_TITLE 컬럼의 데이터 타입을 VARCHAR2(40), LOCATION_ID 컬럼의 데이터 타입을 VARCHAR2(2), LNAME 컬럼의 기본값을 미국으로 변경
...
ALTER TABLE MEMBER_GRADE DROP COLUMN GRADE_CODE; --> 참조되고 있는 컬럼이 있다면 삭제 불가능
ALTER TABLE MEMBER_GRADE DROP COLUMN GRADE_CODE CASCADE CONSTRAINTS;

/*
    1-4) 컬럼명 변경
        ALTER TABLE 테이블명 RENAME COLUNMN 기존 컬럼명 TO 변경할 컬럼명;
*/

-- DEPT_COPY 테이블의 LMANE 컬럼명을 LOCATION_NAME으로 변경
...
/*
    ...
    2-1) 제약조건 추가
        PRYMARY KEY : ADD [CONSTRAINT 제약조건명] PRIMARY KEY(컬럼명);
...
-- DEPT_COPY 테이블의 DEPT_COPY_DEPT_ID_PK 삭제
...
-- 만약 삭제하고자 한다면, 방법 1. 자식테이블을 먼저 삭제한 후 부모테이블을 삭제하는 방법
DROP TABLE MEMBER;
DROP TABLE MEMBER_GRADE;

-- 방법 2. 부모테이블만 삭제하는 제약조건까지 같이 삭제하는 방법
