...
-- 각 부서별(NULL인 경우 부서 없음) 사원의 수, 보너스를 받는 사원의 수, 총 급여, 평균 급여, 최저 급여, 최고 급여 조회 (부서별 오름차순 정렬, 부서 없음이 맨 위에)
SELECT NVL(DEPT_CODE, '부서 없음'), COUNT(*), COUNT(BONUS), SUM(SALARY), ROUND(AVG(NVL(SALARY, 0))), MIN(SALARY), MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST;

-- 성별 별(남자/여자) 사원의 수 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1),1, '남자', 2, '여자') "성별", COUNT(*) AS "사원의 수"
FROM EMPLOYEE
...
/*
    ...
    SELECT문 실행 순서
    SELECT : 5  * | 조회하고자 하는 컬럼명 AS 별칭 | 계산식 | 함수식
    FROM : 1  조회하고자 하는 테이블명
    WHERE : 2  조건식 (연산자들 가지고 기술)
    GROUP BY : 3 그룹 기준에 해당하는 컬럼명 | 계산식 | 함수식
    HAVING : 4 조건식 (그룹 함수를 가지고 기술)
    ORDER BY : 6 컬럼명 | 별칭 | 컬럼 순번 [AS|DESC] [NULLAS FIRST|NULLS LAST]
*/

-- 부서별 평균 급여가 300만원 이상인 직원의 평균 급여를 조회 (부서코드, 평균 급여)
SELECT DEPT_CODE AS "부서 코드", ROUND(AVG(NVL(SALARY, 0))) AS "평균 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING ROUND(AVG(NVL(SALARY, 0))) >= 3000000
ORDER BY DEPT_CODE;
...
/*
    집계 함수
     ...
     - CUBE(컬럼1, 컬럼2) : 컬럼1을 가지고 중간 집계도 내고, 컬럼2를 가지고 중간 집계를 또 냄
*/
...
-- 위 쿼리문 대신 WHERE절에 OR 연산자를 사용해서 처리 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
