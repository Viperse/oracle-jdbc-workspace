...
/*
    INSERT ALL
    두 개 이상의 테이블의 각각 INSERT 하는데 동일한 서브쿼리가 사용되는 경우
...
-- EMPLOYEE 테이블의 입사일 기준으로 2000년 1월 1일 이전에 입사한 사원의 정보는 EMP_OLD 테이블에 삽입하고 2000년 1월 1일 이후에 입사한 사원의 정보는 EMP_NEW 테이블에 삽입한다.
INSERT ALL
    WHEN HIRE_DATE < TO_DATE(20000101) THEN INTO EMP_OLD VALUES(EMP_ID, EMP_NAME, HIRE_DATE, SALARY)
...
-- DEPT_COPY 테이블에서 DEPT_ID가 'D9'인 부서명을 전략기획팀으로 수정
UPDATE DEPT_COPY
...
WHERE EMP_ID IN (SELECT EMP_ID
                 FROM EMP_SALARY
                 JOIN DEPT_COPY ON (DEPT_CODE = DEPT_ID) JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
