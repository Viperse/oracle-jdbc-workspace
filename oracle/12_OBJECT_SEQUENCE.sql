...
/*
    ...
    [MINVAULE 숫자]           --> 최소값 지정 (기본값 1)
    [CYCLE|NOCYCLE]           --> 값 순환 여부 지정 (기본값 NOCYCLE)
    [NOCACHE|CACHE 바이트크기] --> 캐시메모리 할당 (기본값 CACHE 20)
    
    * 캐시메모리
     - 미리 발생된 값들을 생성해서 저장해 두는 공간
     - 매번 호출될 때마다 새로이 번호를 생성하는 게 아니라 캐시메모리 공간에 미리 생성된 값들을 가져다 쓸 수 있음 (속도가 빨라짐)
...
-- WHY? CURRVAL은 마지막으로 성공적으로 수행된 NEXTVAL의 값을 저장해서 보여 주는 임시 값
...
SELECT SEQ_EMPID.NEXTVAL FROM DUAL; -- MAXVAL 초과 에러 발생
...
/*
    ...
    ALTER SEQUENCE 시퀀스명
    [INCREMENT BY 숫자]  --> 몇 씩까지 증가시킬 건지 (기본값 1)
    [MAXVALUE 숫자]      --> 최대값 지정 (기본값 매우 큼)
...
-- 2) 매번 새로운 사번이 발생되는 SEQUENCE를 사용해서 INSERT 구문 작성
