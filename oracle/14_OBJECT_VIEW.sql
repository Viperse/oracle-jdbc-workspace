...
/*
    ...
    * VIEW 옵션들
    ...
    - WTTH READ ONLY : 뷰에 대해 조회만 가능하다. (DML 수행 불가)
*/
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO, 8, 1) = 1 THEN '남' ELSE '여' END 성별, EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) "근무년수"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);

-- 1) 서브쿼리에서 별칭을 부여하는 방법
CREATE OR REPLACE VIEW VW_EMP_JOB
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO, 8, 1) = 1 THEN '남' ELSE '여' END 성별, EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) "근무년수"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO, 8, 1) = 1 THEN '남' ELSE '여' END , EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) 
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
...
/*
    VIEW를 이용해서 DML(INSERT, UPDATE, DELETE) 사용 가능
     - 뷰를 통해서 조작하게 되면 실제 데이터가 담겨 있는 베이스 테이블에 반영됨
*/
CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_CODE, JOB_NAME
   FROM JOB;
   
SELECT * FROM VW_JOB; -- 논리적인 테이블 (실제 데이터가 담겨 있지는 않음)
...
-- UPDATE
UPDATE VW_JOB SET JOB_NAME = '인턴' WHERE JOB_CODE = '7';  -- 에러 발생
...
-- INSERT
INSERT INTO VW_JOB VALUES('인턴'); -- 에러 발생 JOB_CODE에 NULL이 들어올 수 없음
...
SELECT DEPT_CODE, SUM(SALARY) 합계, ROUND(AVG(SALARY)) 평균
FROM EMPLOYEE
GROUP BY DEPT_CODE;
