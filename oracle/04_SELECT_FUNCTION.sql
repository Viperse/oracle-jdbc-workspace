/*
 함수 : 전달된 컬럼값을 읽어들여서 함수를 실행한 결과를 반환
    ...
    >> 함수식을 기술할 수 있는 위치 : SELECT, WHERE, ORDER BY, GROUB BY, HAVING
...
SELECT CONCAT('가나다라','ABCD','1234')FROM DUAL; -- 실행안됨(문자열 2개만 받을 수 있음)
...
-- 사원명, 직급 코드, 기존 급여, 인상된 급여 조회 직급 코드가 J7인 사원은 급여 10% 인상, 직급 코드가 J6인 사원은 급여를 15% 인상, J5인 사람은 급여를 20% 인상, 그 외의 직급은 5% 인상
-- 정렬: 직급 코드 J1부터, 인상된 급여 높은 순서대로
SELECT EMP_NAME, JOB_CODE, SALARY, DECODE(JOB_CODE, 'J7', (SALARY + (SALARY * 0.1)), 'J6', (SALARY + (SALARY * 0.15)), 'J5', (SALARY + (SALARY * 0.2)), (SALARY + (SALARY * 0.05))) AS "인상된 급여"
FROM EMPLOYEE
ORDER BY JOB_CODE, "인상된 급여" DESC NULLS LAST;
...
-- 사번, 사원명, 주민번호, 성별(남, 여) 조회
SELECT EMP_ID, EMP_NAME, EMP_NO, CASE WHEN SUBSTR(EMP_NO, 8, 1) = 1 THEN '남자' WHEN SUBSTR(EMP_NO, 8, 1) = 2 THEN '여자' ELSE '잘못된 주민번호입니다.' END AS "성별"
FROM EMPLOYEE;

-- 사원명, 급여, 급여 등급(1~4등급 조회) 급여 값이 500만원 초과일 경우 1등급, 500만원 이하 350만원 초과일 경우 2등급, 350만원 이하 200만원 초과일 경우 3등급 그 외의 경우 4등급
SELECT EMP_NAME, SALARY, CASE WHEN SALARY > 5000000 THEN '1등급' WHEN SALARY > 3500000 THEN '2등급' WHEN SALARY > 2000000 THEN '3등급' ELSE '4등급' END AS "급여 등급"
FROM EMPLOYEE;
...
-- 전체 사원의 총 급여 합
SELECT TO_CHAR(SUM(SALARY), 'FM999,999,999')  -- FM 붙이면 앞에 공백 제거됨
FROM EMPLOYEE;

-- 부서코드가 D5인 사원들의 총 연봉 합
SELECT TO_CHAR(SUM(SALARY * 12), 'FM999,999,999') AS "부서코드가 D5인 사원들의 연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
-- 전체 사원의 평균 급여 조회
SELECT TO_CHAR(ROUND(AVG(SALARY)), 'FM99,999,999') AS "평균 급여"
FROM EMPLOYEE;
...
/*
  COUNT(*|컬럼|DISTINCT 컬럼)
   ...
   COUNT(컬럼) : 해당 컬럼 값이 NULL이 아닌 행의 개수를 반환
   COUNT(DISTINCT 컬럼) : 해당 컬럼 값의 중복을 제거한 행의 개수를 반환
*/
